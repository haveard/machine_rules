...
def example_programmatic_rules():
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"{i}. {msg}")
        print(
            f"   Category: {category.upper()} | Discount: {discount}% | Credit Limit: ${credit:,}"
        )
    print("-" * 70)
...
"""
    ...
    try:
        ...
        test_customers = [
            ...
            {"income": 120000, "loyalty_years": 2, "name": "Premium by Income"},
            {"income": 80000, "loyalty_years": 4, "name": "Premium by Loyalty"},
            ...
            for rule in execution_set.get_rules():
                if rule.condition(customer):
                    if result_index < len(results):
                        ...
                        print(
                            f"👤 {customer['name']:20} | {segment:10} | Bonus: ${bonus:4}"
                        )
                        if perks:
                            print(f"   Perks: {perks}")
                        result_index += 1
...
def example_complex_business_logic():
    ...
    def excellent_credit_rule(fact: Dict[str, Any]) -> bool:
        ...
        return (
            ...
            and fact.get("debt_to_income", 1.0) <= 0.3
        )
    ...
    def good_credit_rule(fact: Dict[str, Any]) -> bool:
        ...
        return (
            fact.get("credit_score", 0) >= 650
            and fact.get("income", 0) >= 40000
            and fact.get("debt_to_income", 1.0) <= 0.43
        )
    ...
    def manual_review_rule(fact: Dict[str, Any]) -> bool:
        ...
        return fact.get("credit_score", 0) >= 600 and fact.get("income", 0) >= 30000
    ...
    rules = [
        Rule(
            "excellent_credit",
            excellent_credit_rule,
            excellent_credit_action,
            priority=100,
        ),
    ...
    for i, (app, result) in enumerate(zip(applications, results), 1):
        ...
        decision_icon = (
            "✅" if result.get("approved") else "❌" if decision == "DECLINED" else "⏳"
        )

        print(
            f"{decision_icon} {app['name']:20} | Score: {app['credit_score']:3} | "
            f"Income: ${app['income']:,}"
        )
        print(
            f"   Decision: {decision:15} | Rate: {result.get('rate'):.1f}% | "
